...
/* ── CARD MODAL ──────────────────────────────────────────────────────── */
function openCardModal(mode, cardId, deckId) {
  App.cardModal.mode   = mode;
  App.cardModal.cardId = cardId || null;
  App.cardModal.deckId = deckId || App.currentDeckId;

  D.cardModalTitle.textContent = mode === 'edit' ? 'Edit Card' : 'New Card';
  clearCardModal();

  if (mode === 'edit' && cardId) {
    Storage.Cards.get(cardId).then(card => {
  ...
  setTimeout(() => D.cardFrontEditor.focus(), 100);
}

function clearCardModal() {
  D.cardFrontEditor.innerHTML = '';
  D.cardBackEditor.innerHTML  = '';
  D.clozeEditor.value         = '';
  D.cardTags.value            = '';
  D.cardNotes.value           = '';
...
async function gradeCard(grade) {
  ...
  const SESSION_WINDOW = 20 * 60 * 1000; // 20 mins (so 1-hour steps don't force a wait screen)
  const updatedCard = await Storage.Cards.get(card.id);

  if (grade === 0) {
    // Again — requeue near the front so it repeats very soon
    const insertIdx = Math.min(Session.queue.length, Session.index + 3);
    Session.queue.splice(insertIdx, 0, { ...updatedCard });
  } else if ((grade === 1 || grade === 2) && updatedCard.interval === 0
             && updatedCard.dueDate <= Date.now() + SESSION_WINDOW) {
    // Hard or Good learning step — requeue so it shows up this session
